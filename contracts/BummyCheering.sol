...
contract BummyCheering is BummyOwnership{
    ...
    function _triggerCooldown(Bummy storage _bummy) internal {
        ...
        _bummy.cooldownEndTime = uint64(
            block.timestamp + cooldowns[_bummy.cooldownIndex]
        );
    ...
    ///  period has passed.
    function _isReadyToInviteFriend(
        Bummy memory _mom
    ) private view returns (bool) {
        return
            (_mom.cheeringWithId != 0) &&
            (_mom.cooldownEndTime <= block.timestamp);
    }
    ...
    function _isReadyToCheer(Bummy memory _bum) internal view returns (bool) {
        ...
        return
            (_bum.cheeringWithId == 0) &&
            (_bum.cooldownEndTime <= block.timestamp);
    }
    ...
    function canCheerWith(
        ...
        return
            _isValidMatingPair(mom, _momId, dad, _dadId) &&
    ...
    function cheerWith(uint256 _momId, uint256 _dadId) external whenNotPaused {
        ...
        require(_isValidMatingPair(mom, _momId, dad, _dadId));
    ...
    function _cheerWith(uint256 _momId, uint256 _dadId) internal {
        ...
        emit Cheering(_ownerOf(_momId), _momId, _dadId);
    }
